...
def process_segment(args):
  ...
  try:
    ...
    lr = LogReader(url, sort_by_time=True)
    msgs = list(lr)
    ...
    data = [(m.logMonoTime, m.longitudinalPlan) for m in replayed if m.which() == "longitudinalPlan"]
    ...
    ref = pickle.loads(zstd.ZstdDecompressor().decompress(ref_file.read_bytes()))
    diffs = []
...
def main(platform=None, segments_per_platform=1, update_refs=False, all_platforms=False):
...
if __name__ == "__main__":
  ...
  parser.add_argument("--platform", help="Test single platform")
  parser.add_argument("--segments-per-platform", type=int, default=1)
